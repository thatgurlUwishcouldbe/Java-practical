...
public class FlightSchedule {
    ...
    // search by destination توقع
    public void searchFlightByDestin(BSTNode root, String destin) {
        if (root == null) {
            return;
        }
        Flight found = (Flight) root.getData();

        if (found.getDestination().equals(destin)) {
            System.out.println("Found");
        }

        searchFlightByDestin(root.getLeft(), destin);
        searchFlightByDestin(root.getRight(), destin);
    }
    ...
    //Write a method that counts flights departing before a given time توقع
    public int countFlightsBefore( BSTNode root,String time,String des){
        int result =0;
        
        if(root == null)
            return 0;
        
        Flight comp = (Flight) root.getData();
        
        if(comp.getDepartureTime().compareToIgnoreCase(time) < 0 && comp.getDestination().equalsIgnoreCase(des))
            result++;
        
        result += countFlightsBefore(root.getLeft(),time,des);
        result += countFlightsBefore(root.getRight(),time,des);
...
public class FlightSchedule {
    ...
    public int getFlightCount() {
        ...
        return flights.size(flights.getRoot());
    }
